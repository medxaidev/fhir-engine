...
1. **PostgreSQL 不可用** — 上游 `fhir-persistence` 未导出 `PostgresAdapter`，`database.type = 'postgres'` 将抛出运行时错误
2. **fhir-runtime 核心 JSON** — `fhir-runtime@0.8.x` 的 npm 包缺少 bundled core definition JSON，引擎自动使用 `preloadCore: false` 绕过

---

_fhir-engine v0.4.1 — 项目概览_
